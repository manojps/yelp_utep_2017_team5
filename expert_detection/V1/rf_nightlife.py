...
cur.execute("select u1.user_id, u1.review_count, u1.funny, u1.useful, u1.cool, u1.fans, u2.nightlife, u2.friend_count, u2.tips_count, u2.city, u1.elite from _user as u1 join (select t1.user_id, t1.nightlife, t1.friend_count, t1.tips_count, t2.city from secondary_stats_1 as t1 join user_location_temp as t2 on t1.user_id= t2.user_id where city like '%Las Vegas%') as u2 on u1.user_id = u2.user_id;")
...
#features.head()
clf = RandomForestRegressor(n_jobs=6)
...
label.to_csv('rf_nightlife_lasveags.csv', sep='\t')
